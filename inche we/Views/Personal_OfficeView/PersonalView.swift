...
struct PersonalView: View {
    ...
    // MARK: - SWIFDATA
    @Query(sort:\TaskItem.createDate, order: .reverse) private var taskPersonal: [TaskItem]
    ...
    var filteredResult: [TaskItem]{ // reemplace taskOffice con filteredResult para filtrar únicammente por la categoria office
        taskPersonal.filter{$0.category == .Personal}
    }
    ...
    var body: some View {
        NavigationStack{
            ...
            .padding(.horizontal)
            .navigationTitle("New personal task")
            ...
            .toolbar{
                ...
                ToolbarItem(placement: .topBarTrailing){
                    // MARK: - Esta línea de código me permite ocultar el el boton de save en caso que la pantalla este en modo View, en caso contrario deberá mostrarse en modo Create.
                        ...
                        .disabled(title.trimmingCharacters(in: .whitespaces).isEmpty || dueDate < Date())   //nuevo 29072026
            ...
            .sheet(item: $editingTask){ task in //abre la ventana de creación en modo edición para editar
                    EditPersonalTaskView(task: task)
            }
...
//
    ...
    func saveTask(){
        let list = TaskItem(category: .Personal,
                            title: title,
                            createDate: .now,
                            dueDate: dueDate,
